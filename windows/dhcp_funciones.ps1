...
# --- HERRAMIENTAS DE UI Y VALIDACIÓN ---
...
# Nueva Función Exclusiva para Validar IPs
function Test-ValidIP ($IP) {
    # 1. ¿Es formato IP?
    if (-not ($IP -as [System.Net.IPAddress])) { return $false }
    # 2. Bloquear IPs inválidas para hosts (0.0.0.0, Loopback, APIPA)
...
function Configure-Network-DHCP {
    ...
    # Uso de la nueva función para validar IP del Servidor
    do {
        $ServerIP = Read-Host "IP del Servidor (Inicio de Rango)"
        $validIP = Test-ValidIP $ServerIP
        if (-not $validIP) { Write-Warning "IP Invalida (No se permite 0.0.0.0, 127.x o 169.254.x)." }
    } until ($validIP)
    ...
    # Uso de la nueva función para validar IP Final
    do {
        $RangeEnd = Read-Host "Rango Final"
        $validEnd = Test-ValidIP $RangeEnd
        if (-not $validEnd) { Write-Warning "IP Invalida." }
    } until ($validEnd)
    ...
    do {
        ...
        if ([string]::IsNullOrWhiteSpace($Gateway)) { $validGw = $true }
        else {
            $validGw = Test-ValidIP $Gateway
            if (-not $validGw) { Write-Warning "IP Invalida." }
        }
    } until ($validGw)

    do {
        $DNS = Read-Host "DNS (Opcional - Enter para omitir)"
        if ([string]::IsNullOrWhiteSpace($DNS)) { $validDns = $true }
        else {
            $validDns = Test-ValidIP $DNS
            if (-not $validDns) { Write-Warning "IP Invalida." }
        }
    } until ($validDns)

    Write-Host "`n[...] Configurando IP Estatica..." -ForegroundColor Cyan
    try {
        Remove-NetIPAddress -InterfaceAlias $Global:InterfaceAlias -Confirm:$false -ErrorAction SilentlyContinue | Out-Null
    ...
    try {
        Get-DhcpServerv4Scope -ErrorAction SilentlyContinue | Remove-DhcpServerv4Scope -Force -Confirm:$false | Out-Null
